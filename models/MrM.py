...
class STResNet(nn.Module):
    ...
    def _build_stresnet(self, ):
        ...
        self.ext_net = nn.Sequential(
            MoKLayer(self.external_dim, self.nb_flow * self.map_heigh * self.map_width, experts_type="A", gate_type="Linear"),
        )
    ...
    def forward(self, xc, xp, xt, ext):
        ...
        ext_reshaped = ext.unsqueeze(1)
        ext_out = self.ext_net(ext_reshaped)
        ext_out = ext_out.view([-1, self.nb_flow, self.map_heigh, self.map_width])
